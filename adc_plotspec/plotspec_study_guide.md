...
| 2 | `%PLOTSPEC Plot power spectrum and calculate ADC performance metrics` | 帮助文档首行：给函数一个一句话摘要；MATLAB 的 `help` 和 `lookfor` 会优先显示它，这里说明 `PLOTSPEC` 的用途是「Plot power spectrum and calculate ADC performance metrics」。 |
| 3 | `%   This function performs spectral analysis on ADC data and calculates key` | 帮助文档正文：`This function performs spectral analysis on ADC data and calculates key` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 4 | `%   performance metrics including ENOB, SNDR, SFDR, SNR, and THD. It supports` | 帮助文档正文：`performance metrics including ENOB, SNDR, SFDR, SNR, and THD. It supports` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 5 | `%   various windowing functions, oversampling ratio (OSR), coherent averaging,` | 帮助文档带宽说明：`various windowing functions, oversampling ratio (OSR), coherent averaging,` 解释 OSR 或信号带宽的含义；后续 SNR、噪底和带宽线都依赖它。 |
| 6 | `%   and customizable plotting options.` | 帮助文档正文：`and customizable plotting options.` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 7 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 8 | `%   Syntax:` | 帮助文档小节标题：开启调用格式部分；用户执行 `help plotspec` 时用它定位函数怎么调用。 |
| 9 | `%     [enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig)` | 帮助文档调用格式：`[enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig)` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 10 | `%     [enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs)` | 帮助文档调用格式：`[enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs)` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 11 | `%     [enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs, maxCode)` | 帮助文档调用格式：`[enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs, maxCode)` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 12 | `%     [enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs, maxCode, harmonic)` | 帮助文档调用格式：`[enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, Fs, maxCode, harmonic)` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 13 | `%     [enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, 'Name', Value)` | 帮助文档调用格式：`[enob,sndr,sfdr,snr,thd,sigpwr,noi,nsd,h] = PLOTSPEC(sig, 'Name', Value)` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 14 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 15 | `%   Inputs:` | 帮助文档小节标题：开启输入参数部分；用户执行 `help plotspec` 时用它定位每个输入该传什么。 |
| 16 | `%     sig - Signal to be plot, typically the ADC's output data` | 帮助文档条目：说明 `sig` 的用途是「Signal to be plot, typically the ADC's output data」；这是用户理解该参数或输出的依据。 |
| 17 | `%       Vector or Matrix (N_run x N_fft)` | 帮助文档类型/范围说明：`Vector or Matrix (N_run x N_fft)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 18 | `%       Each row represents a separate measurement run for averaging` | 帮助文档正文：`Each row represents a separate measurement run for averaging` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 19 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 20 | `%   Optional Positional Inputs:` | 帮助文档小节标题：开启可选位置参数部分，说明不用写参数名、按顺序传入的参数。 |
| 21 | `%     Fs - Sampling frequency in Hz. Default: 1` | 帮助文档条目：说明 `Fs` 的用途是「Sampling frequency in Hz. Default: 1」；这是用户理解该参数或输出的依据。 |
| 22 | `%       Scalar, positive real number` | 帮助文档类型/范围说明：`Scalar, positive real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 23 | `%     maxCode - Full scale range (max-min). Default: max(sig)-min(sig)` | 帮助文档条目：说明 `maxCode` 的用途是「Full scale range (max-min). Default: max(sig)-min(sig)」；这是用户理解该参数或输出的依据。 |
| 24 | `%       Scalar, positive real number` | 帮助文档类型/范围说明：`Scalar, positive real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 25 | `%     harmonic - Number of harmonics to analyze. Default: 5` | 帮助文档条目：说明 `harmonic` 的用途是「Number of harmonics to analyze. Default: 5」；这是用户理解该参数或输出的依据。 |
| 26 | `%       Scalar, positive integer` | 帮助文档类型/范围说明：`Scalar, positive integer` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 27 | `%       Set negative to exclude harmonics from noise calculation` | 帮助文档正文：`Set negative to exclude harmonics from noise calculation` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 28 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 29 | `%   Name-Value Parameters:` | 帮助文档小节标题：开启 Name-Value 参数部分，说明用 `'参数名', 参数值` 控制算法和绘图选项。 |
| 30 | `%     'OSR' - Oversampling ratio. Default: 1 (no oversampling)` | 帮助文档条目：说明 `'OSR'` 的用途是「Oversampling ratio. Default: 1 (no oversampling)」；这是用户理解该参数或输出的依据。 |
| 31 | `%       Scalar, positive real number` | 帮助文档类型/范围说明：`Scalar, positive real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 32 | `%       Defines signal bandwidth as Fs/(2*OSR)` | 帮助文档带宽说明：`Defines signal bandwidth as Fs/(2*OSR)` 解释 OSR 或信号带宽的含义；后续 SNR、噪底和带宽线都依赖它。 |
| 33 | `%     'window' - Window function. Default: 'hann'` | 帮助文档条目：说明 `'window'` 的用途是「Window function. Default: 'hann'」；这是用户理解该参数或输出的依据。 |
| 34 | `%       String: 'hann' (Hanning window) or 'rect' (Rectangle window)` | 帮助文档类型/范围说明：`String: 'hann' (Hanning window) or 'rect' (Rectangle window)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 35 | `%       Function handle: e.g., @hann, @blackman, @rectwin (requires Signal Processing Toolbox)` | 帮助文档类型/范围说明：`Function handle: e.g., @hann, @blackman, @rectwin (requires Signal Processing Toolbox)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 36 | `%       Alias: 'winType' (deprecated, use 'window')` | 帮助文档兼容性说明：`Alias: 'winType' (deprecated, use 'window')` 告诉用户旧参数名仍可使用，但推荐迁移到新的参数名。 |
| 37 | `%     'maxSignal' - Full scale range (max-min). Default: max(sig)-min(sig)` | 帮助文档条目：说明 `'maxSignal'` 的用途是「Full scale range (max-min). Default: max(sig)-min(sig)」；这是用户理解该参数或输出的依据。 |
| 38 | `%       Scalar, positive real number` | 帮助文档类型/范围说明：`Scalar, positive real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 39 | `%       Alias: 'maxCode' (deprecated, use 'maxSignal')` | 帮助文档兼容性说明：`Alias: 'maxCode' (deprecated, use 'maxSignal')` 告诉用户旧参数名仍可使用，但推荐迁移到新的参数名。 |
| 40 | `%     'sideBin' - Number of extra bins to include on each side of signal peak. Default: 'auto'` | 帮助文档条目：说明 `'sideBin'` 的用途是「Number of extra bins to include on each side of signal peak. Default: 'auto'」；这是用户理解该参数或输出的依据。 |
| 41 | `%       Scalar, non-negative integer or string 'auto'` | 帮助文档类型/范围说明：`Scalar, non-negative integer or string 'auto'` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 42 | `%       Total signal bins = 1 + 2*sideBin (center peak + sideBin on each side)` | 帮助文档自动 `sideBin` 说明：`Total signal bins = 1 + 2*sideBin (center peak + sideBin on each side)` 解释主信号主瓣范围和泄漏边界，避免把信号泄漏误算成噪声。 |
| 43 | `%       'auto': Automatically detects sideBin by simulating ideal spectral leakage with the` | 帮助文档自动 `sideBin` 说明：`'auto': Automatically detects sideBin by simulating ideal spectral leakage with the` 解释主信号主瓣范围和泄漏边界，避免把信号泄漏误算成噪声。 |
| 44 | `%               same window function and finding where it crosses the noise floor` | 帮助文档自动 `sideBin` 说明：`same window function and finding where it crosses the noise floor` 解释主信号主瓣范围和泄漏边界，避免把信号泄漏误算成噪声。 |
| 45 | `%               Adapts to window type (hann, rect, blackman, etc.) and noise characteristics` | 帮助文档正文：`Adapts to window type (hann, rect, blackman, etc.) and noise characteristics` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 46 | `%       Convention: sideBin = 1 for Hanning window if signal is coherent` | 帮助文档自动 `sideBin` 说明：`Convention: sideBin = 1 for Hanning window if signal is coherent` 解释主信号主瓣范围和泄漏边界，避免把信号泄漏误算成噪声。 |
| 47 | `%     'label' - Enable plot annotations. Default: true` | 帮助文档条目：说明 `'label'` 的用途是「Enable plot annotations. Default: true」；这是用户理解该参数或输出的依据。 |
| 48 | `%       Logical (true/false) or numeric (0/1)` | 帮助文档类型/范围说明：`Logical (true/false) or numeric (0/1)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 49 | `%     'assumedSignal' - Override signal power in dB. Default: NaN` | 帮助文档条目：说明 `'assumedSignal'` 的用途是「Override signal power in dB. Default: NaN」；这是用户理解该参数或输出的依据。 |
| 50 | `%       Scalar, real number or NaN` | 帮助文档类型/范围说明：`Scalar, real number or NaN` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 51 | `%     'disp' - Enable plotting. Default: true` | 帮助文档条目：说明 `'disp'` 的用途是「Enable plotting. Default: true」；这是用户理解该参数或输出的依据。 |
| 52 | `%       Logical (true/false) or numeric (0/1)` | 帮助文档类型/范围说明：`Logical (true/false) or numeric (0/1)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 53 | `%       Alias: 'isPlot' (deprecated, use 'disp')` | 帮助文档兼容性说明：`Alias: 'isPlot' (deprecated, use 'disp')` 告诉用户旧参数名仍可使用，但推荐迁移到新的参数名。 |
| 54 | `%     'cutoff' - High-pass cutoff frequency for low-frequency noise removal in Hz. Default: 0` | 帮助文档条目：说明 `'cutoff'` 的用途是「High-pass cutoff frequency for low-frequency noise removal in Hz. Default: 0」；这是用户理解该参数或输出的依据。 |
| 55 | `%       Scalar, non-negative real number` | 帮助文档类型/范围说明：`Scalar, non-negative real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 56 | `%       Alias: 'noFlicker' (deprecated, use 'cutoff')` | 帮助文档兼容性说明：`Alias: 'noFlicker' (deprecated, use 'cutoff')` 告诉用户旧参数名仍可使用，但推荐迁移到新的参数名。 |
| 57 | `%     'nTHD' - Number of harmonics for THD calculation. Default: 5` | 帮助文档条目：说明 `'nTHD'` 的用途是「Number of harmonics for THD calculation. Default: 5」；这是用户理解该参数或输出的依据。 |
| 58 | `%       Scalar, positive integer` | 帮助文档类型/范围说明：`Scalar, positive integer` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 59 | `%     'averageMode' - Averaging mode. Default: 'normal'` | 帮助文档条目：说明 `'averageMode'` 的用途是「Averaging mode. Default: 'normal'」；这是用户理解该参数或输出的依据。 |
| 60 | `%       String: 'normal' (power averaging), 'coherent' (coherent averaging with phase alignment)` | 帮助文档类型/范围说明：`String: 'normal' (power averaging), 'coherent' (coherent averaging with phase alignment)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 61 | `%       Number: 0 (normal), 1 (coherent)` | 帮助文档类型/范围说明：`Number: 0 (normal), 1 (coherent)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 62 | `%       Alias: 'coAvg' (deprecated, use 'averageMode')` | 帮助文档兼容性说明：`Alias: 'coAvg' (deprecated, use 'averageMode')` 告诉用户旧参数名仍可使用，但推荐迁移到新的参数名。 |
| 63 | `%     'NFMethod' - Noise floor estimation method. Default: 'auto'` | 帮助文档条目：说明 `'NFMethod'` 的用途是「Noise floor estimation method. Default: 'auto'」；这是用户理解该参数或输出的依据。 |
| 64 | `%       String: 'auto' (median of all methods), 'median' (median-based), 'mean' (trimmed mean), 'exclude' (exclude harmonics)` | 帮助文档类型/范围说明：`String: 'auto' (median of all methods), 'median' (median-based), 'mean' (trimmed mean), 'exclude' (exclude harmonics)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 65 | `%       Number: 0 (auto), 1 (median-based), 2 (trimmed mean), 3 (exclude harmonics)` | 帮助文档类型/范围说明：`Number: 0 (auto), 1 (median-based), 2 (trimmed mean), 3 (exclude harmonics)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 66 | `%     'dispItem' - Display items selector. Default: 'sfedutrlyhop' (all items)` | 帮助文档条目：说明 `'dispItem'` 的用途是「Display items selector. Default: 'sfedutrlyhop' (all items)」；这是用户理解该参数或输出的依据。 |
| 67 | `%       String or char array where each character (case insensitive) enables a display item:` | 帮助文档类型/范围说明：`String or char array where each character (case insensitive) enables a display item:` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 68 | `%       's' - Signal power text and signal bin marker` | 帮助文档条目：说明 `'s'` 的用途是「Signal power text and signal bin marker」；这是用户理解该参数或输出的依据。 |
| 69 | `%       'f' - Input frequency and sampling frequency (Fin/Fs)` | 帮助文档条目：说明 `'f'` 的用途是「Input frequency and sampling frequency (Fin/Fs)」；这是用户理解该参数或输出的依据。 |
| 70 | `%       'e' - Effective Number of Bits (ENOB)` | 帮助文档条目：说明 `'e'` 的用途是「Effective Number of Bits (ENOB)」；这是用户理解该参数或输出的依据。 |
| 71 | `%       'd' - Signal-to-Noise and Distortion Ratio (SNDR)` | 帮助文档条目：说明 `'d'` 的用途是「Signal-to-Noise and Distortion Ratio (SNDR)」；这是用户理解该参数或输出的依据。 |
| 72 | `%       'u' - Spurious-Free Dynamic Range (SFDR)` | 帮助文档条目：说明 `'u'` 的用途是「Spurious-Free Dynamic Range (SFDR)」；这是用户理解该参数或输出的依据。 |
| 73 | `%       't' - Total Harmonic Distortion (THD)` | 帮助文档条目：说明 `'t'` 的用途是「Total Harmonic Distortion (THD)」；这是用户理解该参数或输出的依据。 |
| 74 | `%       'r' - Signal-to-Noise Ratio (SNR)` | 帮助文档条目：说明 `'r'` 的用途是「Signal-to-Noise Ratio (SNR)」；这是用户理解该参数或输出的依据。 |
| 75 | `%       'l' - Noise floor level` | 帮助文档条目：说明 `'l'` 的用途是「Noise floor level」；这是用户理解该参数或输出的依据。 |
| 76 | `%       'y' - Noise Spectral Density (NSD) and horizontal dash line` | 帮助文档条目：说明 `'y'` 的用途是「Noise Spectral Density (NSD) and horizontal dash line」；这是用户理解该参数或输出的依据。 |
| 77 | `%       'o' - Oversampling Ratio (OSR) and vertical bandwidth line` | 帮助文档条目：说明 `'o'` 的用途是「Oversampling Ratio (OSR) and vertical bandwidth line」；这是用户理解该参数或输出的依据。 |
| 78 | `%       'h' - Harmonic markers` | 帮助文档条目：说明 `'h'` 的用途是「Harmonic markers」；这是用户理解该参数或输出的依据。 |
| 79 | `%       'p' - Maximum spur marker` | 帮助文档条目：说明 `'p'` 的用途是「Maximum spur marker」；这是用户理解该参数或输出的依据。 |
| 80 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 81 | `%   Outputs:` | 帮助文档小节标题：开启输出变量部分；用户执行 `help plotspec` 时用它定位每个返回值代表什么。 |
| 82 | `%     enob - Effective Number of Bits` | 帮助文档条目：说明 `enob` 的用途是「Effective Number of Bits」；这是用户理解该参数或输出的依据。 |
| 83 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 84 | `%       Calculated as (sndr-1.76)/6.02` | 帮助文档正文：`Calculated as (sndr-1.76)/6.02` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 85 | `%     sndr - Signal-to-Noise and Distortion Ratio in dB` | 帮助文档条目：说明 `sndr` 的用途是「Signal-to-Noise and Distortion Ratio in dB」；这是用户理解该参数或输出的依据。 |
| 86 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 87 | `%     sfdr - Spurious-Free Dynamic Range in dB` | 帮助文档条目：说明 `sfdr` 的用途是「Spurious-Free Dynamic Range in dB」；这是用户理解该参数或输出的依据。 |
| 88 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 89 | `%     snr - Signal-to-Noise Ratio in dB` | 帮助文档条目：说明 `snr` 的用途是「Signal-to-Noise Ratio in dB」；这是用户理解该参数或输出的依据。 |
| 90 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 91 | `%     thd - Total Harmonic Distortion in dB` | 帮助文档条目：说明 `thd` 的用途是「Total Harmonic Distortion in dB」；这是用户理解该参数或输出的依据。 |
| 92 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 93 | `%     sigpwr - Signal power in dBFS` | 帮助文档条目：说明 `sigpwr` 的用途是「Signal power in dBFS」；这是用户理解该参数或输出的依据。 |
| 94 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 95 | `%     noi - Noise Floor in dB` | 帮助文档条目：说明 `noi` 的用途是「Noise Floor in dB」；这是用户理解该参数或输出的依据。 |
| 96 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 97 | `%     nsd - Noise Spectral Density in dBFS/Hz` | 帮助文档条目：说明 `nsd` 的用途是「Noise Spectral Density in dBFS/Hz」；这是用户理解该参数或输出的依据。 |
| 98 | `%       Scalar, real number` | 帮助文档类型/范围说明：`Scalar, real number` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 99 | `%     h - Plot handle or empty array if disp=false` | 帮助文档条目：说明 `h` 的用途是「Plot handle or empty array if disp=false」；这是用户理解该参数或输出的依据。 |
| 100 | `%       Graphics handle or []` | 帮助文档类型/范围说明：`Graphics handle or []` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 101 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 102 | `%   Examples:` | 帮助文档小节标题：开启示例代码部分，下面的注释和代码演示不同使用场景。 |
| 103 | `%     % Basic usage with default parameters (uses built-in Hanning window)` | 示例中的注释：`% Basic usage with default parameters (uses built-in Hanning window)` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 104 | `%     [enob,sndr,sfdr] = plotspec(sig);` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig);` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 105 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 106 | `%     % Specify sampling frequency and full scale` | 示例中的注释：`% Specify sampling frequency and full scale` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 107 | `%     [enob,sndr,sfdr] = plotspec(sig, 100e6, 2^16);` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 100e6, 2^16);` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 108 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 109 | `%     % Use oversampling with rectangle window (no toolbox required)` | 示例中的注释：`% Use oversampling with rectangle window (no toolbox required)` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 110 | `%     [enob,sndr,sfdr] = plotspec(sig, 'OSR', 32, 'window', 'rect');` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 'OSR', 32, 'window', 'rect');` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 111 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 112 | `%     % Use other window functions (requires Signal Processing Toolbox)` | 示例中的注释：`% Use other window functions (requires Signal Processing Toolbox)` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 113 | `%     [enob,sndr,sfdr] = plotspec(sig, 'OSR', 32, 'window', @blackman);` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 'OSR', 32, 'window', @blackman);` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 114 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 115 | `%     % Use trimmed mean for noise floor estimation` | 示例中的注释：`% Use trimmed mean for noise floor estimation` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 116 | `%     [enob,sndr,sfdr] = plotspec(sig, 'NFMethod', 'mean');` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 'NFMethod', 'mean');` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 117 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 118 | `%     % Multiple runs with coherent averaging` | 示例中的注释：`% Multiple runs with coherent averaging` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 119 | `%     sig = ones(10,1)*sin(2*pi*0.1*(0:1023)) + randn(10, 1024)*0.01; % 10 runs of 1024 samples` | 帮助文档正文：`sig = ones(10,1)*sin(2*pi*0.1*(0:1023)) + randn(10, 1024)*0.01; % 10 runs of 1024 samples` 会显示在 `help plotspec` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 120 | `%     [enob,sndr,sfdr] = plotspec(sig, 'averageMode', 'coherent');` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 'averageMode', 'coherent');` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 121 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 122 | `%     % Disable plotting and use assumed signal power` | 示例中的注释：`% Disable plotting and use assumed signal power` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 123 | `%     [enob,sndr,sfdr] = plotspec(sig, 'disp', false, 'assumedSignal', -3);` | 帮助文档调用格式：`[enob,sndr,sfdr] = plotspec(sig, 'disp', false, 'assumedSignal', -3);` 展示 `plotspec` 可以这样调用；它说明参数从少到多的写法和输出变量顺序。 |
| 124 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 125 | `%   Notes:` | 帮助文档小节标题：开启注意事项部分，下面记录算法假设、单位约定和特殊行为。 |
| 126 | `%     - Signal can be provided as a row vector, column vector, or matrix` | 帮助文档注意事项：`- Signal can be provided as a row vector, column vector, or matrix` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 127 | `%     - For matrix input, each row is treated as a separate measurement` | 帮助文档注意事项：`- For matrix input, each row is treated as a separate measurement` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 128 | `%     - The FFT length is determined by the number of columns (or rows if column vector)` | 帮助文档注意事项：`- The FFT length is determined by the number of columns (or rows if column vector)` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 129 | `%     - Coherent averaging ('coAvg') aligns phase before averaging to lower noise floor` | 帮助文档注意事项：`- Coherent averaging ('coAvg') aligns phase before averaging to lower noise floor` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 130 | `%     - The noise floor is calculated within the signal band [0, Fs/(2*OSR)]` | 帮助文档注意事项：`- The noise floor is calculated within the signal band [0, Fs/(2*OSR)]` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 131 | `%     - Setting harmonic &lt; 0 removes harmonics from both the analysis and display` | 帮助文档注意事项：`- Setting harmonic < 0 removes harmonics from both the analysis and display` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 132 | `%     - dBFS = 0 referred to a full-scale sinewave signal` | 帮助文档注意事项：`- dBFS = 0 referred to a full-scale sinewave signal` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 133 | `%     - Built-in windows ('hann', 'rect') do not require Signal Processing Toolbox` | 帮助文档注意事项：`- Built-in windows ('hann', 'rect') do not require Signal Processing Toolbox` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 134 | `%     - Custom window function handles (e.g., @blackman) require Signal Processing Toolbox` | 帮助文档注意事项：`- Custom window function handles (e.g., @blackman) require Signal Processing Toolbox` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 135 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 136 | `%   See also: alias, sinfit, fft, window` | 帮助文档相关函数：`See also: alias, sinfit, fft, window` 告诉读者还可以查看这些函数，方便继续追踪相关工具。 |
| 137 | <空行> | 空行，用来把帮助文本和后面的正式代码分开；MATLAB 执行时会忽略。 |
| 138 | `% Input parsing and validation` | 源码分段注释：`Input parsing and validation` 标明下面进入输入检查/输入解析阶段，先验证参数是否合法。 |
...
| 151 | `% Old parameter names (for backward compatibility)` | 源码分段注释：`Old parameter names (for backward compatibility)` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 156 | `% New parameter names (aliases with higher priority)` | 源码分段注释：`New parameter names (aliases with higher priority)` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 162 | `% Other parameters` | 源码注释：`Other parameters` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
...
| 171 | `% Extract parsed parameters` | 源码分段注释：`Extract parsed parameters` 标明下面把 `inputParser` 的结果取到局部变量里，供后续算法直接使用。 |
...
| 180 | `% Convert NFMethod from string to numeric if needed` | 噪底方法注释：`Convert NFMethod from string to numeric if needed` 标明下面处理噪底估计方法；这些选择会影响 SNR 和噪声谱密度。 |
...
| 196 | `% Handle aliased parameters (new names override old names)` | 源码注释：`Handle aliased parameters (new names override old names)` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
| 197 | `% averageMode/coAvg` | 源码分段注释：`averageMode/coAvg` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
| 198 | `if ~isequal(p.Results.averageMode, 'normal')` | 检查新版 `averageMode` 是否被用户显式设置；新版参数优先于旧版 `coAvg`。 |
| 199 | `    % New name has priority if not default` | 分支意图注释：`New name has priority if not default` 说明当前分支是在新参数和旧参数之间做优先级选择。 |
...
| 211 | `    % Fall back to old name` | 分支意图注释：`Fall back to old name` 说明当前分支是在新参数和旧参数之间做优先级选择。 |
...
| 215 | `% maxSignal/maxCode` | 源码分段注释：`maxSignal/maxCode` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 222 | `% window/winType - use windowFunc as internal variable to avoid shadowing built-in` | 源码分段注释：`window/winType - use windowFunc as internal variable to avoid shadowing built-in` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 229 | `% disp/isPlot - convert to logical` | 源码分段注释：`disp/isPlot - convert to logical` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 236 | `% cutoff/noFlicker` | 源码分段注释：`cutoff/noFlicker` 标明下面处理新旧参数名兼容关系；新名字优先，旧名字用于兼容老脚本。 |
...
| 243 | `% Parse dispItem flags` | 显示开关注释：`Parse dispItem flags` 标明下面解析图上显示项目的字母开关。 |
...
| 258 | `% Determine data dimensions and FFT length` | 数据准备注释：`Determine data dimensions and FFT length` 标明下面整理输入尺寸、FFT 点数或频率坐标，这是频谱分析的基础。 |
| 259 | `% Convert column vector to row vector if needed` | 数据准备注释：`Convert column vector to row vector if needed` 标明下面整理输入尺寸、FFT 点数或频率坐标，这是频谱分析的基础。 |
...
| 269 | `% Calculate number of positive frequency bins` | 数据准备注释：`Calculate number of positive frequency bins` 标明下面整理输入尺寸、FFT 点数或频率坐标，这是频谱分析的基础。 |
| 270 | `Nd2 = floor(N_fft/2)+1;` | 计算单边频谱点数；实信号 FFT 只需要 DC 到 Nyquist 的正频率部分。 |
| 271 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 272 | `% Generate frequency axis` | 数据准备注释：`Generate frequency axis` 标明下面整理输入尺寸、FFT 点数或频率坐标，这是频谱分析的基础。 |
| 273 | `freq = (0:(Nd2-1))/N_fft*Fs;` | 生成频率坐标轴，把 FFT bin 编号换算成 Hz。 |
| 274 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 275 | `% Generate window function` | 窗函数注释：`Generate window function` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 276 | `if ischar(windowFunc)` | 判断窗函数是否用内置字符串形式指定。 |
| 277 | `    % Use embedded window functions (no toolbox required)` | 窗函数注释：`Use embedded window functions (no toolbox required)` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
...
| 287 | `    % Use function handle (requires Signal Processing Toolbox)` | 窗函数注释：`Use function handle (requires Signal Processing Toolbox)` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
...
| 300 | `% Initialize spectrum accumulator and measurement counter` | 频谱累加注释：`Initialize spectrum accumulator and measurement counter` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
...
| 305 | `    % Skip empty data` | 频谱累加注释：`Skip empty data` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
...
| 309 | `    % Normalize to full scale, remove DC, and apply window` | 窗函数注释：`Normalize to full scale, remove DC, and apply window` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
...
| 315 | `        % Coherent averaging: align phase before averaging` | 频谱累加注释：`Coherent averaging: align phase before averaging` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
| 316 | `        tspec = fft(tdata);` | 对当前记录做 FFT，得到复数频谱；复数相位用于相干平均。 |
| 317 | `        tspec(1) = 0;  % Remove DC component` | 清除 DC bin，避免直流偏置被误认为主信号。 |
| 318 | `        % Find fundamental signal bin in signal band` | 相干平均/谐波注释：`Find fundamental signal bin in signal band` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
| 319 | `        [~, bin] = max(abs(tspec(1:floor(N_fft/2/OSR))));` | 在带内寻找幅度最大的 FFT bin，作为基波信号位置。 |
| 320 | `        % Guard against bin = 1 (DC bin) which would cause division by zero` | 源码注释：`Guard against bin = 1 (DC bin) which would cause division by zero` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
...
| 327 | `        % Phase alignment: rotate spectrum to align fundamental phase` | 相干平均/谐波注释：`Phase alignment: rotate spectrum to align fundamental phase` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
| 328 | `        phasor = conj(phi);` | 构造相位旋转因子，用来把基波旋转到共同参考相位。 |
| 329 | `        marker = zeros(1,N_fft);` | 创建标记数组，记录哪些 bin 已按谐波关系处理过。 |
| 330 | `        % Apply phase shift to harmonics (accounting for aliasing)` | 相干平均/谐波注释：`Apply phase shift to harmonics (accounting for aliasing)` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
| 331 | `        for iter2 = 1:N_fft` | 内层循环遍历谐波序列，把基波、谐波及其折叠位置逐个相位对齐。 |
| 332 | `            J = (bin-1)*iter2;` | 计算第 `iter2` 阶谐波对应的未折叠 bin 编号；`bin-1` 是从 0 开始的 bin。 |
| 333 | `            % Determine if harmonic is in even or odd Nyquist zone` | 相干平均/谐波注释：`Determine if harmonic is in even or odd Nyquist zone` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
| 334 | `            if(mod(floor(J/N_fft*2),2) == 0)` | 判断谐波位于偶数还是奇数 Nyquist 区；奇偶区决定折叠是否镜像。 |
| 335 | `                % Even zone: normal aliasing` | 相干平均/谐波注释：`Even zone: normal aliasing` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 342 | `                % Odd zone: mirrored aliasing` | 相干平均/谐波注释：`Odd zone: mirrored aliasing` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 352 | `        % Apply phase shift to non-harmonic components` | 相干平均/谐波注释：`Apply phase shift to non-harmonic components` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 362 | `        % Power averaging: accumulate power spectrum` | 频谱累加注释：`Power averaging: accumulate power spectrum` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
...
| 369 | `% Normalize spectrum based on averaging method` | 指标计算注释：`Normalize spectrum based on averaging method` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
| 370 | `if(coAvg)` | 根据 `coAvg` 选择相干平均路径或普通功率平均路径。 |
| 371 | `    % Coherent averaging: take magnitude squared after sum, scale by number of runs` | 频谱累加注释：`Coherent averaging: take magnitude squared after sum, scale by number of runs` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
| 372 | `    spec = abs(spec).^2/(N_fft^2)*16/ME^2;` | 相干平均后先取幅度平方，再按 FFT 长度、满量程正弦标定和运行次数平方归一化。 |
| 373 | `else` | 条件分支的兜底路径：前面条件都不满足时执行。 |
| 374 | `    % Power averaging: scale by number of runs` | 频谱累加注释：`Power averaging: scale by number of runs` 标明下面处理每次运行的 FFT 结果，决定是功率平均还是相位对齐后的相干平均。 |
...
| 382 | `% Remove flicker noise (1/f noise) if requested` | 频谱修正注释：`Remove flicker noise (1/f noise) if requested` 标明下面处理低频噪声、主峰亚 bin 估计或泄漏告警。 |
...
| 387 | `% Find signal bin and refine using parabolic interpolation` | 频谱修正注释：`Find signal bin and refine using parabolic interpolation` 标明下面处理低频噪声、主峰亚 bin 估计或泄漏告警。 |
...
| 392 | `% Parabolic interpolation for sub-bin frequency accuracy` | 频谱修正注释：`Parabolic interpolation for sub-bin frequency accuracy` 标明下面处理低频噪声、主峰亚 bin 估计或泄漏告警。 |
...
| 398 | `% Warn if signal is off-bin, indicating likely spectrum leakage` | 频谱修正注释：`Warn if signal is off-bin, indicating likely spectrum leakage` 标明下面处理低频噪声、主峰亚 bin 估计或泄漏告警。 |
...
| 406 | `% Auto-detect sideBin if set to 'auto'` | 自动 `sideBin` 注释：`Auto-detect sideBin if set to 'auto'` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
| 407 | `if ischar(sideBin) &amp;&amp; strcmp(sideBin, 'auto')` | 若 `sideBin` 为 `auto`，进入自动检测主瓣宽度流程。 |
| 408 | `    % Step 1: Generate ideal spectrum at bin_r frequency` | 自动 `sideBin` 注释：`Step 1: Generate ideal spectrum at bin_r frequency` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
| 409 | `    % Create synthetic sinewave with unit amplitude` | 自动 `sideBin` 注释：`Create synthetic sinewave with unit amplitude` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 413 | `    % Apply same window and normalization as actual data` | 窗函数注释：`Apply same window and normalization as actual data` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 414 | `    ideal_signal = ideal_signal .* win / sqrt(mean(win.^2));` | 对理想正弦使用同样窗函数和能量归一化，使其泄漏主瓣可与实测谱比较。 |
| 415 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 416 | `    % Compute FFT to get ideal spectrum shape` | 自动 `sideBin` 注释：`Compute FFT to get ideal spectrum shape` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 420 | `    % Scale ideal spectrum to match actual signal magnitude at peak` | 自动 `sideBin` 注释：`Scale ideal spectrum to match actual signal magnitude at peak` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
| 421 | `    % This ensures we compare spectral leakage at the same signal strength` | 源码注释：`This ensures we compare spectral leakage at the same signal strength` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
...
| 425 | `    % Step 2: Estimate noise floor using median (robust to signal peak outliers)` | 自动 `sideBin` 注释：`Step 2: Estimate noise floor using median (robust to signal peak outliers)` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 429 | `    % Step 3: Find crossing points where ideal spectrum meets noise floor` | 自动 `sideBin` 注释：`Step 3: Find crossing points where ideal spectrum meets noise floor` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 433 | `    % Search outward from peak until ideal spectrum drops below noise floor` | 自动 `sideBin` 注释：`Search outward from peak until ideal spectrum drops below noise floor` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 438 | `        % Check if both left and right bins are below noise floor in ideal spectrum` | 自动 `sideBin` 注释：`Check if both left and right bins are below noise floor in ideal spectrum` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 443 | `            % Both sides below noise floor, use previous sb` | 自动 `sideBin` 注释：`Both sides below noise floor, use previous sb` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 449 | `    % If never broke out, use maximum` | 自动 `sideBin` 注释：`If never broke out, use maximum` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 455 | `% Calculate signal power including side bins` | 功率积分注释：`Calculate signal power including side bins` 标明下面决定哪些谱线算主信号或谐波，这会影响 SNDR、SNR、THD 和噪声统计。 |
...
| 458 | `% Override with assumed signal power if provided` | 功率积分注释：`Override with assumed signal power if provided` 标明下面决定哪些谱线算主信号或谐波，这会影响 SNDR、SNR、THD 和噪声统计。 |
...
| 464 | `% Remove harmonics from spectrum for display if harmonic &lt; 0` | 相干平均/谐波注释：`Remove harmonics from spectrum for display if harmonic < 0` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 472 | `% Plot spectrum if requested` | 绘图注释：`Plot spectrum if requested` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
| 473 | `if(dispPlot)` | 若开启绘图，则进入频谱绘制流程。 |
| 474 | `    % Use linear or log scale depending on OSR` | 源码注释：`Use linear or log scale depending on OSR` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
...
| 483 | `    % Mark signal bins if label enabled` | 绘图注释：`Mark signal bins if label enabled` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 492 | `    % Mark harmonics if requested` | 相干平均/谐波注释：`Mark harmonics if requested` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 502 | `% Calculate SNDR and SFDR` | 指标计算注释：`Calculate SNDR and SFDR` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
| 503 | `% Save signal bin value for SFDR calculation` | 指标计算注释：`Save signal bin value for SFDR calculation` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 508 | `% Remove signal and DC from spectrum for noise/distortion calculation` | 指标计算注释：`Remove signal and DC from spectrum for noise/distortion calculation` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 514 | `% Find largest spur for SFDR` | 指标计算注释：`Find largest spur for SFDR` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 520 | `% Mark maximum spur on plot` | 自动 `sideBin` 注释：`Mark maximum spur on plot` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 526 | `% Calculate noise floor using all methods and select per NFMethod` | 噪底方法注释：`Calculate noise floor using all methods and select per NFMethod` 标明下面处理噪底估计方法；这些选择会影响 SNR 和噪声谱密度。 |
...
| 529 | `% Method 1: Median-based estimation (robust to spurs)` | 指标计算注释：`Method 1: Median-based estimation (robust to spurs)` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
| 530 | `if(N_run == 1)` | 根据运行次数选择中位数噪声估计的校正系数。 |
| 531 | `    % Mn = 0.4549364231; % theoretical value of the median of chi-squared distribution, but not working well` | 指标计算注释：`Mn = 0.4549364231; % theoretical value of the median of chi-squared distribution, but not working well` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 537 | `% Method 2: Trimmed mean (removes top/bottom 5%)` | 指标计算注释：`Method 2: Trimmed mean (removes top/bottom 5%)` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 540 | `% Method 3: Exclude harmonics from noise calculation` | 相干平均/谐波注释：`Method 3: Exclude harmonics from noise calculation` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 549 | `    % Auto: median of all methods` | 指标计算注释：`Auto: median of all methods` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
| 550 | `    noi = median([noi_median, noi_mean, noi_exclude]);` | 自动模式取三种噪声估计值的中位数，避免单一方法偏差过大。 |
| 551 | `    % Warn if results differ significantly (&gt;25% or ~1dB)` | 指标计算注释：`Warn if results differ significantly (>25% or ~1dB)` 标明下面统计噪声、失真、最大杂散或谐波功率，用来得到 ADC 性能指标。 |
...
| 566 | `% Calculate THD by summing harmonic power` | 相干平均/谐波注释：`Calculate THD by summing harmonic power` 标明下面围绕基波相位旋转频谱，并考虑谐波关系或别名折叠。 |
...
| 577 | `% Finalize plot formatting and annotations` | 绘图注释：`Finalize plot formatting and annotations` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
| 578 | `if(dispPlot)` | 若开启绘图，则进入频谱绘制流程。 |
| 579 | `    % Set axis limits based on noise floor` | 自动 `sideBin` 注释：`Set axis limits based on noise floor` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 583 | `        % Draw signal bandwidth limit` | 绘图注释：`Draw signal bandwidth limit` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 587 | `        % Determine text position based on scale and signal location` | 绘图注释：`Determine text position based on scale and signal location` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 600 | `        % Format sampling frequency with SI prefixes` | 绘图注释：`Format sampling frequency with SI prefixes` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 613 | `        % Format input frequency with SI prefixes` | 绘图注释：`Format input frequency with SI prefixes` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 627 | `        % Display performance metrics` | 绘图注释：`Display performance metrics` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 658 | `        % Display additional metrics and noise floor line` | 自动 `sideBin` 注释：`Display additional metrics and noise floor line` 标明下面用理想正弦泄漏谱和实际噪底比较，决定主信号主瓣范围。 |
...
| 673 | `            % Position signal power label to avoid signal peak` | 功率积分注释：`Position signal power label to avoid signal peak` 标明下面决定哪些谱线算主信号或谐波，这会影响 SNDR、SNR、THD 和噪声统计。 |
...
| 688 | `    % Set axis labels and title` | 绘图注释：`Set axis labels and title` 标明下面把计算结果画到频谱图上，包括坐标、标记、指标文字、单位格式或标题。 |
...
| 702 | `% Assign output variables with new names` | 输出注释：`Assign output variables with new names` 标明下面把内部计算变量映射到函数声明中的输出变量。 |
...
| 716 | `% Nested functions for embedded window generation (no toolbox required)` | 窗函数注释：`Nested functions for embedded window generation (no toolbox required)` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 717 | `    function w = rectwin_emb(N)` | 定义嵌套矩形窗函数；嵌套函数只能在 `plotspec` 内部直接调用。 |
| 718 | `        % RECTWIN_EMB Embedded rectangle (boxcar) window function` | 窗函数注释：`RECTWIN_EMB Embedded rectangle (boxcar) window function` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 719 | `        %   w = RECTWIN_EMB(N) returns an N-point rectangle window in a row vector` | 窗函数注释：`w = RECTWIN_EMB(N) returns an N-point rectangle window in a row vector` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 720 | `        %   This is a simple embedded implementation that doesn't require` | 源码注释：`This is a simple embedded implementation that doesn't require` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
| 721 | `        %   the Signal Processing Toolbox` | 窗函数注释：`the Signal Processing Toolbox` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
...
| 726 | `        % HANNWIN Embedded Hanning window function` | 窗函数注释：`HANNWIN Embedded Hanning window function` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 727 | `        %   w = HANNWIN(N) returns an N-point Hanning (raised cosine) window` | 窗函数注释：`w = HANNWIN(N) returns an N-point Hanning (raised cosine) window` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 728 | `        %   in a row vector. This is a simple embedded implementation that` | 源码注释：`in a row vector. This is a simple embedded implementation that` 不会被 MATLAB 执行；它用来提示下面代码块的目的，帮助读者理解这段代码为什么存在。 |
| 729 | `        %   doesn't require the Signal Processing Toolbox` | 窗函数注释：`doesn't require the Signal Processing Toolbox` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 730 | `        %` | 注释空行：用于把源码注释分成段落；MATLAB 不执行它，只帮助读者分清逻辑块。 |
| 731 | `        %   The Hanning window is defined as:` | 窗函数注释：`The Hanning window is defined as:` 标明下面生成或说明窗函数；窗函数会影响频谱泄漏、主瓣宽度和功率积分。 |
| 732 | `        %   w(n) = 0.5 * (1 - cos(2*pi*n/(N-1))) for n = 0, 1, ..., N-1` | 嵌套函数注释：`w(n) = 0.5 * (1 - cos(2*pi*n/(N-1))) for n = 0, 1, ..., N-1` 标明下面是文件内部自带的窗函数实现，让 `plotspec` 不依赖额外工具箱也能运行。 |
...
| 2 | `%ALIAS Calculate aliased frequency after sampling` | 帮助文档首行：给函数一个一句话摘要；MATLAB 的 `help` 和 `lookfor` 会优先显示它，这里说明 `ALIAS` 的用途是「Calculate aliased frequency after sampling」。 |
| 3 | `%   This function returns the aliased frequency of a signal after sampling,` | 帮助文档正文：`This function returns the aliased frequency of a signal after sampling,` 会显示在 `help alias` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 4 | `%   accounting for different Nyquist zones. The aliasing follows the pattern` | 帮助文档正文：`accounting for different Nyquist zones. The aliasing follows the pattern` 会显示在 `help alias` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 5 | `%   where signals in even Nyquist zones (0, 2, 4...) alias normally, while` | 帮助文档正文：`where signals in even Nyquist zones (0, 2, 4...) alias normally, while` 会显示在 `help alias` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 6 | `%   signals in odd Nyquist zones (1, 3, 5...) alias with spectral inversion.` | 帮助文档正文：`signals in odd Nyquist zones (1, 3, 5...) alias with spectral inversion.` 会显示在 `help alias` 中；它用来说明函数用途、能力、参数含义或使用方式。 |
| 7 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 8 | `%   Syntax:` | 帮助文档小节标题：开启调用格式部分；用户执行 `help alias` 时用它定位函数怎么调用。 |
| 9 | `%     fal = ALIAS(fin, fs)` | 帮助文档调用格式：`fal = ALIAS(fin, fs)` 展示 `alias` 的基本用法；输入原始频率和采样频率，输出折叠后的频率。 |
| 10 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 11 | `%   Inputs:` | 帮助文档小节标题：开启输入参数部分；用户执行 `help alias` 时用它定位每个输入该传什么。 |
| 12 | `%     fin - Signal frequency before sampling` | 帮助文档条目：说明 `fin` 的用途是「Signal frequency before sampling」；这是用户理解该参数或输出的依据。 |
| 13 | `%       Scalar or Vector` | 帮助文档类型/范围说明：`Scalar or Vector` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 14 | `%     fs - Sampling frequency. Must be a positive real number.` | 帮助文档条目：说明 `fs` 的用途是「Sampling frequency. Must be a positive real number.」；这是用户理解该参数或输出的依据。 |
| 15 | `%       Scalar` | 帮助文档类型/范围说明：`Scalar` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 16 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 17 | `%   Outputs:` | 帮助文档小节标题：开启输出变量部分；用户执行 `help alias` 时用它定位每个返回值代表什么。 |
| 18 | `%     fal - Aliased signal frequency after sampling ` | 帮助文档条目：说明 `fal` 的用途是「Aliased signal frequency after sampling」；这是用户理解该参数或输出的依据。 |
| 19 | `%       Scalar or Vector (same size as fin)` | 帮助文档类型/范围说明：`Scalar or Vector (same size as fin)` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 20 | `%       Range: [0, fs/2]` | 帮助文档类型/范围说明：`Range: [0, fs/2]` 规定上一项可以接受或返回什么类型、范围或编码方式。 |
| 21 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 22 | `%   Examples:` | 帮助文档小节标题：开启示例代码部分，下面的注释和代码演示不同使用场景。 |
| 23 | `%     % Signal at 0.7*fs aliases to 0.3*fs in first Nyquist zone (mirrored)` | 示例中的注释：`% Signal at 0.7*fs aliases to 0.3*fs in first Nyquist zone (mirrored)` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 24 | `%     fal = alias(70, 100)  % Returns 30 ` | 帮助文档调用格式：`fal = alias(70, 100)  % Returns 30 ` 展示 `alias` 的基本用法；输入原始频率和采样频率，输出折叠后的频率。 |
| 25 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 26 | `%     % Signal at 1.3*fs aliases to 0.3*fs in second Nyquist zone (normal)` | 示例中的注释：`% Signal at 1.3*fs aliases to 0.3*fs in second Nyquist zone (normal)` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 27 | `%     fal = alias(130, 100)  % Returns 30 ` | 帮助文档调用格式：`fal = alias(130, 100)  % Returns 30 ` 展示 `alias` 的基本用法；输入原始频率和采样频率，输出折叠后的频率。 |
| 28 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 29 | `%     % Multiple frequencies` | 示例中的注释：`% Multiple frequencies` 是写给示例代码读者看的说明；它解释下一行示例想演示的场景。 |
| 30 | `%     fal = alias([30 70 130], 100)  % Returns [30 30 30]` | 帮助文档调用格式：`fal = alias([30 70 130], 100)  % Returns [30 30 30]` 展示 `alias` 的基本用法；输入原始频率和采样频率，输出折叠后的频率。 |
| 31 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 32 | `%   Notes:` | 帮助文档小节标题：开启注意事项部分，下面记录算法假设、单位约定和特殊行为。 |
| 33 | `%     - Nyquist zone n is defined as [(n)*fs/2, (n+1)*fs/2]` | 帮助文档注意事项：`- Nyquist zone n is defined as [(n)*fs/2, (n+1)*fs/2]` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 34 | `%     - Even zones: direct aliasing (fal = mod(fin,fs))` | 帮助文档注意事项：`- Even zones: direct aliasing (fal = mod(fin,fs))` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 35 | `%     - Odd zones: mirrored aliasing (fal = fs - mod(fin,fs))` | 帮助文档注意事项：`- Odd zones: mirrored aliasing (fal = fs - mod(fin,fs))` 记录使用前提、单位约定或特殊行为，避免误读计算结果。 |
| 36 | `%` | 注释空行：用于把帮助文本分成段落；MATLAB 不执行它，但 `help` 显示时会更清楚。 |
| 37 | `%   See also: findFin, findBin` | 帮助文档相关函数：`See also: findFin, findBin` 告诉读者还可以查看这些函数，方便继续追踪相关工具。 |
| 38 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 39 | `    % Input validation` | 源码分段注释：`Input validation` 标明下面进入输入检查/输入解析阶段，先验证参数是否合法。 |
...
| 48 | `    % Determine Nyquist zone (0-based indexing)` | 别名折叠注释：`Determine Nyquist zone (0-based indexing)` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
| 49 | `    % Zone 0: [0, fs/2], Zone 1: [fs/2, fs], Zone 2: [fs, 3*fs/2], etc.` | 别名折叠注释：`Zone 0: [0, fs/2], Zone 1: [fs/2, fs], Zone 2: [fs, 3*fs/2], etc.` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
| 50 | `    nyquistZone = floor(fin / fs * 2);` | 计算 0-based Nyquist 区编号；每个区宽度是 `fs/2`。 |
| 51 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 52 | `    % Calculate base frequency offset within the Nyquist zone` | 别名折叠注释：`Calculate base frequency offset within the Nyquist zone` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
| 53 | `    baseOffset = fin - floor(fin / fs) * fs;` | 计算输入频率在一个采样周期 `fs` 内的余量，类似取模。 |
| 54 | <空行> | 空行，用来把不同逻辑段分开；MATLAB 执行时会忽略。 |
| 55 | `    % Apply aliasing rule based on even/odd Nyquist zone` | 别名折叠注释：`Apply aliasing rule based on even/odd Nyquist zone` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
| 56 | `    % Even zones (0,2,4,...): normal aliasing` | 别名折叠注释：`Even zones (0,2,4,...): normal aliasing` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
| 57 | `    % Odd zones (1,3,5,...): mirrored aliasing` | 别名折叠注释：`Odd zones (1,3,5,...): mirrored aliasing` 标明下面把频率折叠回第一 Nyquist 带，并按奇偶 Nyquist 区决定是否镜像。 |
